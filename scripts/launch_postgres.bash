...
function print_help {
    ...
    printf "Launch a containerized PostgreSQL database for harden-file-transfer-rs\n"
    ...
    printf "    POSTGRES_DB: User for postgres. Default: 's3server'\n"
...
# Section Parent Path
BINARY_PATH="$( cd -- "$(dirname "${SCRIPT_CALLNAME}")/../" >/dev/null 2>&1 ; pwd -P )"
readonly BINARY_PATH
BINARY_VERSION="$(grep -m1 '^version' "${BINARY_PATH}/Cargo.toml" | awk -F "\"" '{ print $2 }' )"
readonly BINARY_VERSION
printf "%s (v%s, harden-file-transfer-rs-server v%s)\n" "${SCRIPT_NAME}" "${SCRIPT_VERSION}" "${BINARY_VERSION}"\
...
# Check if a custom database name has been set, otherwise default to 's3server'
readonly DB_NAME=${POSTGRES_DB:="s3server"}
...
readonly CONTAINER_NAME="harden-file-transfer-rs-server-db"
...
printf "Creating Database if not available\n"
...
cd "${BINARY_PATH}/migrations" || exit;
